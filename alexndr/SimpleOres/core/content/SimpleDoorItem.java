...
public class SimpleDoorItem extends Item
{
    ...
    public SimpleDoorItem(String doorMaterial)
    {
        super();
    ...
    public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
    {
        ...
        else
        {
            ...
            {
                block = Content.copper_door_block;
            }
            else if(this.material == "onyx")
            {
            	block = Content.onyx_door_block;
            }
            else
            {
            	block = null;
    ...
    public static void placeDoorBlock(World par0World, int par1, int par2, int par3, int par4, Block par5Block)
    {
        ...
        int i1 = (par0World.getBlock(par1 - b0, par2, par3 - b1).isNormalCube() ? 1 : 0) + (par0World.getBlock(par1 - b0, par2 + 1, par3 - b1).isNormalCube() ? 1 : 0);
        int j1 = (par0World.getBlock(par1 + b0, par2, par3 + b1).isNormalCube() ? 1 : 0) + (par0World.getBlock(par1 + b0, par2 + 1, par3 + b1).isNormalCube() ? 1 : 0);
